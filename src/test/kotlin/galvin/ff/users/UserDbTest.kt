...
class UserDbTest{
    ...
    @Test fun should_deactivate_role(){
        for( database in databases ) {
            ...
            val shouldBeInactive = userdb.retrieveRole(activeToInactive.name)
                    ?: throw Exception("Failed to load role from database")
    ...
    @Test fun should_store_and_retrieve_user() {
        for( database in databases ) {
            ...
            val (userDB, user) = testObjects(database)
    ...
    @Test fun should_store_and_retrieve_all_users(){
        for( database in databases ) {
            ...
            val (userDB, _, _, roles) = testObjects(database)
            val expectedCount = 10
    ...
    @Test fun should_retrieve_uuid_by_login(){
        for( database in databases ) {
            ...
            val (userDB, user, user2) = testObjects(database)
            userDB.storeUser(user)
    ...
    @Test fun should_lock_and_unlock_user_by_uuid(){
        for( database in databases ) {
            ...
            val (userDB, toBeLocked, neverLocked) = testObjects(database)
    ...
    fun testObjects(database: DbConnection): UserDbTestObjects{
        val userDB = database.randomUserDB()
        val roles = generateRoles(userdb = userDB)
        val user1 = generateUser(roles)
        val user2 = generateUser(roles)

        return UserDbTestObjects(userDB, user1, user2, roles )
    }

    class UserDbTestObjects(private val userDB: UserDB, private val user1: User, private val user2: User, private val roles: List<Role> ){
